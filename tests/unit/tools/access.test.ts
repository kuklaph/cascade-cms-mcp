...
describe("api_read_access_rights tool", () => {
  test("happy path: calls client.readAccessRights with identifier", async () => {
    ...
    const tool = findTool(tools, "api_read_access_rights");
  ...
  test("library throws: returns isError response", async () => {
    ...
    expect(firstText(result)).toContain("api_read_access_rights");
  });
});
...
describe("api_edit_access_rights tool", () => {
  test("happy path: calls client.editAccessRights with identifier + accessRightsInformation", async () => {
    ...
    registerAccessTools(server as any, client);
    const tool = findTool(tools, "api_edit_access_rights");
  ...
  test("library throws: returns isError response", async () => {
    ...
    expect(result.isError).toBe(true);
    expect(firstText(result)).toContain("api_edit_access_rights");
  });
});
