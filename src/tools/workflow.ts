 ...
 *
 *   api_read_workflow_settings        — read folder workflow config
 *   api_edit_workflow_settings        — update folder workflow config
 *   api_read_workflow_information     — read an in-flight workflow
 *   api_perform_workflow_transition   — advance a workflow step
 *
...
export function registerWorkflowTools(
  ...
  registerCascadeTool(server, {
    name: "api_read_workflow_settings",
...
Examples:
  ...
  - Don't use when: You want to inspect an in-flight workflow — use api_read_workflow_information.
...
Examples:
  ...
  - Don't use when: You want to advance an in-flight workflow — use api_perform_workflow_transition.
  - Don't use when: You only need to read — use api_read_workflow_settings.
...
Examples:
  ...
  - Don't use when: You want workflow policy — use api_read_workflow_settings.
...
Error Handling:
  ...
  registerCascadeTool(server, {
    name: "api_perform_workflow_transition",
    title: "Perform Workflow Transition",
    description: buildCascadeToolDescription(
      `Advance an in-flight workflow to its next step (approve, reject, publish, etc.).

Executes a named action against an active workflow. The workflowId and actionIdentifier come from a prior api_read_workflow_information call — the tool does not enumerate actions itself. A transitionComment is recommended so reviewers understand the decision. Once the final step is executed, Cascade may publish, delete, or otherwise commit the change associated with the workflow.

Args:
  - workflowTransitionInformation (object, required):
    - workflowId (string, required): The active workflow's id (from api_read_workflow_information)
...
Examples:
  ...
  - Don't use when: You don't yet know which actions are valid — call api_read_workflow_information first.
