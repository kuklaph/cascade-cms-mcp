 ...
 *
 *   api_check_out — lock an asset for exclusive editing
 *   api_check_in  — release a lock and commit a comment
 *
...
Check-out creates a working copy of the asset that only the authenticated user can edit; other users see the previously committed version until check-in. Required for some asset types (especially files and binary content types) before api_edit will succeed. The response includes a workingCopyIdentifier that represents the locked working copy for subsequent calls. Always pair with api_check_in when editing finishes to release the lock.
...
Examples:
  ...
  - Don't use when: You've finished editing — use api_check_in to release.
  - Don't use when: Read-only operations — checkout isn't needed for api_read.
